...
export default function Login() {
  ...
  const handleIDPError = (error: string) => {
    setLocalError(error);
  };
    ...
    >
      <section
        className={cn(
          ...
          "items-center justify-center py-3 sm:py-5 lg:py-6",
        )}
      >
        <div
          className={cn(
        ...
        >
          <div className="pointer-events-none absolute inset-0">
            <div
              className={cn(
                "absolute -left-12 top-0 h-48 w-48",
                "rounded-full bg-red-500/10 blur-3xl",
              )}
            />
            <div
              className={cn(
                "absolute bottom-0 right-0 h-56 w-56",
                "rounded-full bg-amber-300/10 blur-3xl",
              )}
            />
            <div
              className={cn(
                "absolute inset-0",
                "bg-[linear-gradient(135deg,rgba(255,255,255,0.10),transparent_45%,rgba(255,255,255,0.03))]",
              )}
            />
          </div>
          ...
          <div
            className={cn("relative flex items-center", "p-6 sm:p-8 lg:p-10")}
          >
            <div className="mx-auto w-full max-w-xl">
              <div className="mb-7 space-y-2">
                <span
                  className={cn(
                    "inline-flex backdrop-blur",
                    "rounded-full border border-[hsl(var(--border)/0.7)]",
