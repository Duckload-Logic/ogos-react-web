...
const VerifyPage: React.FC = () => {
  ...
  const [timeLeft, setTimeLeft] = useState<number>(() => {
    ...
    const storedExpiry = localStorage.getItem(`verify_expiry_${registrationId}`);
    if (storedExpiry) {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      localStorage.setItem(`verify_expiry_${registrationId}`, newExpiry.toString());
      setTimeLeft(180);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md z-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="glass-card space-y-8 p-8 relative overflow-hidden">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-muted-foreground font-medium">
              We sent an OTP to <span className="text-foreground font-bold">{maskEmail(email)}</span>
            </p>
          </div>

          {error && (
            <div className="p-4 rounded-xl bg-danger/10 border border-danger/20 text-danger text-sm flex items-center gap-2 animate-in shake duration-300">
              <AlertCircle size={18} />
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleVerify} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="flex h-12 w-full rounded-lg border border-input bg-background px-10 py-2 text-center text-2xl font-bold tracking-[0.5em] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring transition-all"
                  placeholder=""
                />
              </div>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 h-12 bg-primary text-primary-foreground hover:bg-primary/90 rounded-lg font-semibold transition-all disabled:opacity-50"
            >
            ...
            <div className="text-sm text-muted-foreground">
              ...
              {timeLeft > 0 ? (
                ...
                <button
                  ...
                  disabled={resending}
                  className="text-primary hover:underline font-bold inline-flex items-center gap-1 transition-all"
                >
