...
const GLASS_CARD =
  "overflow-hidden rounded-[18px] border border-white/55 bg-white/35 shadow-[0_12px_30px_rgba(15,23,42,0.06)] backdrop-blur-2xl backdrop-saturate-150 dark:border-white/10 dark:bg-white/[0.045] dark:shadow-[0_12px_30px_rgba(0,0,0,0.28)]";
...
const STATUS_CARD_META: Record<string, StatusCardMeta> = {
  pending: {
    ...
    card: "border-amber-300/30 bg-gradient-to-br from-amber-50/55 via-white/35 to-white/25 shadow-amber-100/20 dark:border-amber-400/15 dark:from-amber-400/6 dark:via-white/[0.035] dark:to-white/[0.02]",
    glow: "bg-amber-200/20 dark:bg-amber-400/8",
    iconBox:
      "border-amber-300/30 bg-amber-50/45 text-amber-700 dark:border-amber-400/15 dark:bg-amber-400/8 dark:text-amber-200",
  },
  scheduled: {
    icon: CalendarClock,
    label: "text-sky-700/90 dark:text-sky-200",
    card: "border-sky-300/30 bg-gradient-to-br from-sky-50/55 via-white/35 to-white/25 shadow-sky-100/20 dark:border-sky-400/15 dark:from-sky-400/6 dark:via-white/[0.035] dark:to-white/[0.02]",
    glow: "bg-sky-200/20 dark:bg-sky-400/8",
    iconBox:
      "border-sky-300/30 bg-sky-50/45 text-sky-700 dark:border-sky-400/15 dark:bg-sky-400/8 dark:text-sky-200",
  },
  completed: {
    icon: CheckCircle2,
    label: "text-emerald-700/90 dark:text-emerald-200",
    card: "border-emerald-300/30 bg-gradient-to-br from-emerald-50/55 via-white/35 to-white/25 shadow-emerald-100/20 dark:border-emerald-400/15 dark:from-emerald-400/6 dark:via-white/[0.035] dark:to-white/[0.02]",
    glow: "bg-emerald-200/20 dark:bg-emerald-400/8",
    iconBox:
      "border-emerald-300/30 bg-emerald-50/45 text-emerald-700 dark:border-emerald-400/15 dark:bg-emerald-400/8 dark:text-emerald-200",
  },
  cancelled: {
    icon: XCircle,
    label: "text-rose-700/90 dark:text-rose-200",
    card: "border-rose-300/30 bg-gradient-to-br from-rose-50/55 via-white/35 to-white/25 shadow-rose-100/20 dark:border-rose-400/15 dark:from-rose-400/6 dark:via-white/[0.035] dark:to-white/[0.02]",
    glow: "bg-rose-200/20 dark:bg-rose-400/8",
    iconBox:
      "border-rose-300/30 bg-rose-50/45 text-rose-700 dark:border-rose-400/15 dark:bg-rose-400/8 dark:text-rose-200",
  },
  rejected: {
    icon: Ban,
    label: "text-red-700/90 dark:text-red-200",
    card: "border-red-300/30 bg-gradient-to-br from-red-50/55 via-white/35 to-white/25 shadow-red-100/20 dark:border-red-400/15 dark:from-red-400/6 dark:via-white/[0.035] dark:to-white/[0.02]",
    glow: "bg-red-200/20 dark:bg-red-400/8",
    iconBox:
      "border-red-300/30 bg-red-50/45 text-red-700 dark:border-red-400/15 dark:bg-red-400/8 dark:text-red-200",
  },
  rescheduled: {
    icon: RotateCcw,
    label: "text-violet-700/90 dark:text-violet-200",
    card: "border-violet-300/30 bg-gradient-to-br from-violet-50/55 via-white/35 to-white/25 shadow-violet-100/20 dark:border-violet-400/15 dark:from-violet-400/6 dark:via-white/[0.035] dark:to-white/[0.02]",
    glow: "bg-violet-200/20 dark:bg-violet-400/8",
    iconBox:
      "border-violet-300/30 bg-violet-50/45 text-violet-700 dark:border-violet-400/15 dark:bg-violet-400/8 dark:text-violet-200",
  },
  "no-show": {
    icon: UserX,
    label: "text-slate-700/90 dark:text-slate-200",
    card: "border-slate-300/35 bg-gradient-to-br from-slate-50/65 via-white/35 to-white/25 shadow-slate-100/20 dark:border-slate-400/15 dark:from-slate-400/6 dark:via-white/[0.035] dark:to-white/[0.02]",
    glow: "bg-slate-200/20 dark:bg-slate-400/8",
    iconBox:
      "border-slate-300/35 bg-slate-50/55 text-slate-700 dark:border-slate-400/15 dark:bg-slate-400/8 dark:text-slate-200",
  },
  default: {
    icon: CalendarDays,
    label: "text-primary/90 dark:text-white",
    card: "border-primary/20 bg-gradient-to-br from-primary/8 via-white/35 to-white/25 shadow-primary/10 dark:border-white/10 dark:from-white/[0.055] dark:via-white/[0.035] dark:to-white/[0.02]",
    glow: "bg-primary/15 dark:bg-white/8",
    iconBox:
      "border-primary/20 bg-primary/8 text-primary dark:border-white/10 dark:bg-white/[0.055] dark:text-white",
  },
};
...
export default function StudentAppointments() {
  ...
  return (
    <div className="relative isolate overflow-hidden rounded-[28px]">
      <div className="pointer-events-none absolute -left-24 -top-24 h-72 w-72 rounded-full bg-slate-300/10 blur-3xl dark:bg-slate-500/10" />
      <div className="pointer-events-none absolute right-0 top-10 h-80 w-80 rounded-full bg-primary/8 blur-3xl dark:bg-primary/10" />
      <div className="pointer-events-none absolute bottom-0 left-1/3 h-72 w-72 rounded-full bg-sky-200/8 blur-3xl dark:bg-sky-400/8" />
            ...
            {appointmentStatuses.map((stat: AppointmentStatus, index: number) => {
              ...
              return (
                <Card
                  ...
                  className={cn(
                    GLASS_CARD,
                    "animate-fade-in-up group relative h-[124px] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_16px_36px_rgba(15,23,42,0.08)]",
                    statusMeta.card,
                  )}
                  style={{
